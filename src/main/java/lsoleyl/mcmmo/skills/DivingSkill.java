...
public class DivingSkill implements ISkill {
    public static final LinearProbabilityProgression airRestoreChance = new LinearProbabilityProgression(0.001);
    ...
    public void printDescription(ChatWriter chat, int level) {
        ...
        chat.writeMessage(ChatFormat.formatEffectValue("Last breath chance", ChatFormat.formatPercent(airRestoreChance.getValue(level))));
    }
    ...
    public void printHelp(ChatWriter chat) {
        ...
        chat.writeMessage(" Long diving training pays off. You get a chance to refill your lungs with air upon taking damage from drowning." +
                " The chance to refill your lungs increases by " + ChatFormat.formatPercent(airRestoreChance.incrementPerLevel) +
                " per level.");
    }
}
